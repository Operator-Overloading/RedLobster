...
namespace Lobster
{
	...
	public:
		...
		virtual std::pair<float,float> GetPos() const { return {_x,_y}; }
	...
	{
	public:
		MouseLeftEvent(const float x,const float y) : _x(x),_y(y),_in(false) {}
	...
	{
	public:
		MouseEnteredEvent(const float x,const float y) : _x(x),_y(y),_in(true) {}
